...
| 04 `remove_nested_and_writable_executables` | partial | Host W^X/permission routing, Android-derived guest DNS preservation/fallback, local-only Play setup, real ARM64 package transactions for 11/12 baselines, 12-distro build framework, embedded X11 lifecycle refactor, runtime script scanning, and native 16 KB ELF rebuilding are implemented. Provenance passes for Alpine, Void, Chimera, Manjaro, Ubuntu, Kali, and Arch; Debian, openSUSE, Deepin, and Parrot have incomplete upstream records, and Fedora lacks a final accepted artifact. Refreshed Ivarna APK ELF/ZIP plus bundletool AAB alignment checks pass; Play payload provenance and clean device E2E remain blockers. See `worker_reports/04_remove_nested_and_writable_executables_report.md`. |
