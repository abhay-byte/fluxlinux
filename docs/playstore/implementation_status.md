...
| 04 `remove_nested_and_writable_executables` | partial | Host W^X/permission routing, Android-derived guest DNS preservation/fallback, local-only Play setup, real Alpine `apk` baseline transaction, 12-distro build framework, embedded X11 lifecycle refactor, and runtime script scanning are implemented. Alpine provenance passes; Debian provenance is incomplete and ten distro baselines remain unbuilt. Device E2E, bundletool validation, and two 16 KB ELF alignments remain blockers; see `worker_reports/04_remove_nested_and_writable_executables_report.md`. |
