...
| 04 `remove_nested_and_writable_executables` | pass / 04R closure | Play release boundary is closed to seven staged distro modules plus `runtime_host`; Fedora, Void, openSUSE, Deepin, and Parrot are deferred and excluded from the Play graph. Common/Ivarna raw hashes remain separate from Play hashes. Play provenance, size, AAB/APKS structure, host-artifact, runtime-script, 16 KiB, Gradle test, and debug/release gates pass. Poco X6 Pro evidence covers host delivery, Alpine install, seven-card terminal filtering, online Pulse, visible XFCE, X11 stop/restart, and offline local-only desktop startup with networking disabled. Worker 05 was not started. See `worker_reports/04_remove_nested_and_writable_executables_report.md`. |
