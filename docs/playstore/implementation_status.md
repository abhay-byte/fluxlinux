...
| 04 `remove_nested_and_writable_executables` | partial | Host W^X/permission routing, Android-derived guest DNS preservation/fallback, local-only Play setup, real Alpine/Void/Chimera/Ubuntu ARM64 baseline transactions, 12-distro build framework, embedded X11 lifecycle refactor, runtime script scanning, and native 16 KB ELF rebuilding are implemented. Debian provenance is incomplete and Fedora, openSUSE, Deepin, Manjaro, Kali, Parrot, and Arch baselines remain unbuilt. Refreshed Ivarna APK ELF/ZIP plus bundletool AAB alignment checks pass; Play payload provenance and clean device E2E remain blockers. See `worker_reports/04_remove_nested_and_writable_executables_report.md`. |
