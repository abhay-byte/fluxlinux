...
# ─── UNINSTALL MODE ──────────────────────────────────────────────────────
if [ "$1" = "uninstall" ]; then
    echo "FluxLinux Pro: Uninstalling App Development Environment..."
    ...
    echo "FluxLinux Pro: App Development Environment Uninstalled."
    exit 0
fi
# ─── END UNINSTALL MODE ──────────────────────────────────────────────────
...
echo "FluxLinux Pro: Setting up App Development Environment (Android + Flutter + React Native)..."
...
echo "FluxLinux Pro: Checking for broken Java packages..."
if dpkg -l | grep -q "^iU.*openjdk\|^iF.*openjdk"; then
    echo "FluxLinux Pro: Found broken Java packages, removing..."
    dpkg --remove --force-all openjdk-21-jre-headless openjdk-21-jre openjdk-21-jdk-headless openjdk-21-jdk default-jre-headless default-jre default-jdk-headless default-jdk 2>/dev/null || true
    apt autoremove -y 2>/dev/null || true
    echo "FluxLinux Pro: Broken packages removed"
fi
...
echo "FluxLinux Pro: Installing Java Development Kit..."
export DEBIAN_FRONTEND=noninteractive
...
if apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" openjdk-21-jdk 2>/dev/null; then
    JAVA_VERSION="21"
    echo "FluxLinux Pro: OpenJDK 21 installed successfully"
else
    # Fallback: Install without postinst scripts (workaround for /proc issue)
    echo "FluxLinux Pro: Standard installation failed, using workaround method..."
    ...
    echo "FluxLinux Pro: Configuring Java alternatives..."
    ...
    if [ -d "$JAVA_HOME" ]; then
        ...
        echo "FluxLinux Pro: Java alternatives configured"
    else
        echo "FluxLinux Pro: Warning - Could not find Java installation at $JAVA_HOME"
    fi
    
    JAVA_VERSION="21"
    echo "FluxLinux Pro: OpenJDK 21 installed via workaround method"
fi
...
if [ -n "$LIBJLI_PATH" ]; then
    ...
    echo "FluxLinux Pro: Added $LIBJLI_DIR to LD_LIBRARY_PATH"
fi

# Verify Java installation
if command -v java >/dev/null 2>&1 && java -version 2>&1 | grep -q "openjdk"; then
    echo "FluxLinux Pro: Java Status: $(java -version 2>&1 | head -1)"
elif [ -f "/usr/lib/jvm/java-21-openjdk-arm64/bin/java" ]; then
    echo "FluxLinux Pro: Java binaries installed but alternatives not configured"
    echo "FluxLinux Pro: Run 'update-alternatives --config java' to configure"
else
    echo "FluxLinux Pro: Warning - Java installation may have failed"
fi
...
if [ ! -f "$SDK_ROOT/cmdline-tools/latest/bin/sdkmanager" ]; then
    echo "FluxLinux Pro: Installing Android SDK..."
    # Clean partial install
    if [ -d "$SDK_ROOT" ]; then
        echo "FluxLinux Pro: Found partial/corrupt Android SDK. Cleaning..."
        rm -rf "$SDK_ROOT"
    fi
    ...
    echo "FluxLinux Pro: Downloading Android Command Line Tools..."
    ...
    echo "FluxLinux Pro: Accepting Android Licenses..."
...
# Install Components
echo "FluxLinux Pro: Installing Platform Tools, SDK & NDK..."
...
if [ -d "$SDK_ROOT/cmdline-tools/latest-2" ]; then
    echo "FluxLinux Pro: Fixing cmdline-tools path..."
...
echo "FluxLinux Pro: Cleaning up inconsistent NDK installations..."
...
echo "FluxLinux Pro: Installing Platform Tools, SDK 34/35/36..."
...
echo "FluxLinux Pro: Patching Android SDK CMake/Ninja (Wrapper Script)..."
...
echo "FluxLinux Pro: Verifying CMake Wrapper Content:"
...
echo "FluxLinux Pro: Configuring local projects..."
...
echo "FluxLinux Pro: Patching Android SDK ADB/Fastboot (Wrapper Script)..."
...
install_arm64_ndk() {
    ...
    echo "FluxLinux Pro: Checking NDK $NDK_VER ($NDK_RELEASE)..."
...
echo "FluxLinux Pro: Installing ARM64 NDKs (static/musl)..."
...
if [ ! -f "$FLUTTER_ROOT/bin/flutter" ]; then
    echo "FluxLinux Pro: Installing Flutter SDK (Stable)..."
    # Clean partial
    if [ -d "$FLUTTER_ROOT" ]; then
        echo "FluxLinux Pro: Found partial Flutter. Cleaning..."
        rm -rf "$FLUTTER_ROOT"
    fi
    ...
    echo "FluxLinux Pro: Flutter already installed."
...
echo "FluxLinux Pro: Setting Flutter Android SDK..."
...
echo "FluxLinux Pro: Flutter already installed."
echo "FluxLinux Pro: Setting Flutter Android SDK..."
echo "FluxLinux Pro: Fixing permissions..."
...
if command -v zsh >/dev/null 2>&1; then
    ...
    if ! grep -q "/opt/flutter/bin" "$ZSHRC" 2>/dev/null; then
        ...
        echo "FluxLinux Pro: Added Flutter/Android SDK paths to .zshrc"
    fi
fi
...
if [ -f "/opt/gradle/bin/gradle" ]; then
    ...
    if [ "$INSTALLED_VER" = "$GRADLE_VER" ]; then
        echo "FluxLinux Pro: Gradle $GRADLE_VER already installed."
    else
        echo "FluxLinux Pro: Updating Gradle from $INSTALLED_VER to $GRADLE_VER..."
        rm -rf /opt/gradle
        rm -f /tmp/gradle.zip
    fi
fi
...
if [ ! -f "/opt/gradle/bin/gradle" ] || [ "$(/opt/gradle/bin/gradle --version 2>/dev/null | grep 'Gradle' | head -1 | awk '{print $2}')" != "$GRADLE_VER" ]; then
    echo "FluxLinux Pro: Installing Gradle $GRADLE_VER..."
    ...
    if ! unzip -t /tmp/gradle.zip >/dev/null 2>&1; then
        echo "FluxLinux Pro: Downloaded file corrupt, retrying..."
        rm -f /tmp/gradle.zip
    ...
    echo "FluxLinux Pro: Gradle $GRADLE_VER installed."
fi
...
echo "FluxLinux Pro: Installing ARM64 Native Build Tools..."
...
echo "FluxLinux Pro: Installing SDK 35 ARM64 Build Tools (35.0.2)..."
...
echo "FluxLinux Pro: Installing SDK 36 ARM64 Build Tools (36.1.0)..."
...
# Verify ARM64 aapt2
echo "FluxLinux Pro: Verifying ARM64 aapt2..."
...
echo "FluxLinux Pro: Configuring Environment (React Native/Android)..."
...
echo "FluxLinux Pro: Creating global symlinks..."
...
echo "FluxLinux Pro: Ensuring correct permissions for $TARGET_USER..."
...
echo "FluxLinux Pro: App Development Setup Complete!"
